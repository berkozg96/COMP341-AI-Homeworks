...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            for i in state[1]:
                leftCornerList.append(i)
            if leftCornerList == []:
                return []
            if not self.walls[nextx][nexty]:
                nextState = (nextx, nexty)
                cost = 1
                if nextState in leftCornerList:
                    leftCornerList.remove(nextState)
                successors.append(((nextState, leftCornerList), action, cost))
        self._expanded += 1 # DO NOT CHANGE
        return successors
...
def cornersHeuristic(state, problem):
    ...
    if state[0] in corners:
        return 0
    for leftCorners in state[1]:
        heuristic.append(mazeDistance(state[0], leftCorners, problem.startingGameState))
...
def foodHeuristic(state, problem):
    ...
    if state in foodGrid.asList():
        return 0

    for leftFood in foodGrid.asList():
        heuristic.append(mazeDistance(position, leftFood, problem.startingGameState))
    if not len(heuristic) == 0:
